...
teacherSchema.statics.saltAndHashPassword = async function (password) { //mã hóa và hash mật khẩu
...
teacherSchema.statics.validateByCredentials = function (email, password) {//check password
    let User = this;
...
teacherSchema.pre('save', async function (next) {//check password when save create student and save in db
    let user = this;
