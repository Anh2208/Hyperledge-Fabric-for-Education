...
//create new student
export const createStudent = async (req, res) => { //pass trong blockchain chưa mã hóa
    ...
    try {
        ...
        // check user exists
        const checkLedger = (await contract.evaluateTransaction('CheckUserLedger', req.body.email)).toString('utf8');
        ...
        if (checkLedger || checkWallet) {
            res.status(500).json({ success: false, message: "Failed to create student. User exists!!!" });
            return;
        }
        ...
        await create_user(req.body.email);
...
export const createTeacher = async (req, res) => {
    ...
    try {
        ...
        if (checkLedger || checkWallet) {
            res.status(500).json({ success: false, message: "Failed to create teacher. User exists!!!" });
            return;
        }
        ...
        res.status(200).json({ success: true, message: "Create teacher successfully!!!", data: saveTeacher });
    } catch (error) {
...
export const createAdmin = async (req, res) => {
    ...
    try {
        ...
        if (checkLedger || checkWallet) {
            res.status(500).json({ success: false, message: "Failed to create admin. User exists!!!" });
            return;
        }
        ...
        res.status(200).json({ success: true, message: "Create admin successfully!!!", data: saveAdmin });
    } catch (err) {
        res.status(500).json({ success: false, message: "Failed to create admin. Try again" });
    }
}
...
async function registerStudentInLedger(req, ID, Pass) {
    ...
    try {
        ...
        await contract.submitTransaction('CreateStudent', ID, req.body.email, req.body.mssv, req.body.name, req.body.sex, Pass);
...
async function registerTeacherInLedger(req, ID, Pass) {
    ...
    try {
        ...
        await contract.submitTransaction('CreateTeacher', ID, req.body.email, req.body.msgv, req.body.name, req.body.sex, Pass);
...
async function registerAdminInLedger(req, ID, Pass) {
    ...
    try {
        ...
        await contract.submitTransaction('CreateAdmin', ID, req.body.email, req.body.adminMS, req.body.name, req.body.date, req.body.sdt, Pass);
