...
export const createSubject = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ success: true, message: "Create subject successfully!!!", data: saveSubject });
    } catch (error) {
        res.status(500).json({ success: false, message: "Failed to create Subject. Please try again" });
    }
};
...
export const updateSubject = async (req, res) => {
    ...
    try {
        const updateSubject = await Subject.findByIdAndUpdate(id, { $set: req.body, }, { new: true });
